...
3. **`.env` wiring**: compose switches to explicit `--env-file infra/compose/.env` (and `.env.example` documents it); CI no longer relies on repo-root `.env` copy. **Verified 2026-08-14 on the live runner**: shell-exported vars (`set -a; . ./.env`) were dropped before `docker compose up` in the smoke job — the API container booted with empty `ADMIN_API_KEY` and GM-9's fail-closed check killed it. Fix landed on the GM-9 branch (a5eb7b1): copy `.env.example` into `infra/compose/.env` so compose interpolates from the project dir deterministically. GM-12 formalizes this as `--env-file` + a CI assertion that resolved config has no default secrets (M20).
